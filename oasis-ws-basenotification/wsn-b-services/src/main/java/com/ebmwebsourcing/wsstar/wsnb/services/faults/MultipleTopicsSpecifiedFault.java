...
public class MultipleTopicsSpecifiedFault extends AbsWSStarFault {
	...
	public MultipleTopicsSpecifiedFault(Locale lang, String descr) throws WsnbException, WsrfbfException {
		super();
		this.wsnFault = RefinedWsnbFactory.getInstance().createMultipleTopicsSpecifiedFaultType(new Date());
		this.init(lang, descr);
	}
	
	public MultipleTopicsSpecifiedFault(Document faultAsDocument) throws WsnbException, WsrfbfException {
		super();
		this.wsnFault = RefinedWsnbFactory.getInstance().getWsnbReader().readMultipleTopicsSpecifiedFaultType(faultAsDocument);
		this.init();
	}
	
	@Override
	public final Document getDocumentFragment() throws WsnbException {
		return RefinedWsnbFactory.getInstance().getWsnbWriter().writeMultipleTopicsSpecifiedFaultTypeAsDOM(
				(MultipleTopicsSpecifiedFaultType) this.wsnFault);
	}
}
