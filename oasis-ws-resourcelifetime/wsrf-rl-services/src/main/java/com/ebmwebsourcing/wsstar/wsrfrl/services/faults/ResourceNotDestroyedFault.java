 ...
 *
 */
public class ResourceNotDestroyedFault extends AbsWSStarFault {

	private static final long serialVersionUID = 1L;
		
	public ResourceNotDestroyedFault(Locale lang, String descr) throws WsrfrlException, WsrfbfException {
		super();
		this.wsnFault = RefinedWsrfrlFactory.getInstance().createResourceNotDestroyedFaultType(new Date());
		this.init(lang,descr);			
	}
	
	public ResourceNotDestroyedFault(Document faultAsDocument) throws WsrfrlException, WsrfbfException {
		super();
		this.wsnFault = RefinedWsrfrlFactory.getInstance().getWsrfrlReader().readResourceNotDestroyedFaultType(faultAsDocument);
		this.init();	
	}	

	@Override
	public final Document getDocumentFragment() throws WsrfrlException {
		return RefinedWsrfrlFactory.getInstance().getWsrfrlWriter().writeResourceNotDestroyedFaultTypeAsDOM(
				(ResourceNotDestroyedFaultType)this.wsnFault);
	}
}
